...
<head>
   ...
   <!-- for rendering -->
    <?php
if (isset($_GET['teacher_id'], $_GET['teacher_name'], $_GET['course_name'])) {
    $teacher_id = $_GET['teacher_id'];
    $teacher_name = urldecode($_GET['teacher_name']);
    ...
    for ($i = 1; $i <= 22; $i++) {  // Update to loop through all 22 rows
        if (isset($_GET['row' . $i])) {
            ...
            if (is_numeric($selected_value)) { // Ensure it's a valid numeric value
                $total_score += intval($selected_value);  // Add the value to the total score
            }
        }
    }
...
/* header end */
        ...
        .wholeTable tbody tr:nth-child(odd) {
            background-color: #f2f2f2;
            /* Light gray for odd rows */
        }

        /* Hover Effect for Rows */
        .wholeTable tbody tr:hover {
        ...
        .wholeTable tbody td input[type="radio"] {
            margin: 0 10px;
...
<body>
    ...
    <form action="EvaluationTable.php" method="GET" onsubmit="handleSubmit(event)">
    ...
    <script>
        let x=0;
        const averresult=document.getElementById('result');
        function handleSubmit(event) {
            ...
            // Iterate through formData to collect all inputs
            formData.forEach((value, key) => {
                results[key] = value;
             x+=value/22;
             console.log(x);
             averresult.textContent = `${x} out of 5`;

            });

            console.log(results); // Log the filled data
            alert(JSON.stringify(results, null, 2)); // Optional: Show data in an alert
        }
        // to see 
